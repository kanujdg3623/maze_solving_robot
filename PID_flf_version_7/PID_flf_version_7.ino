...
void replay()
{
  ...
  while(path[i]!='E')
  {
    if(digitalRead(LEFT_IR)==WHITE || digitalRead(RIGHT_IR)==WHITE)
    {
      maneuver(FORWARD,FORWARD,225,225);
      delay(400);
      if(path[i]=='S')
      ...
      do
      {
        qtr.readCalibrated(sensorValues);
        delay(1);
      }while(sensorValues[LEFT-1]>800 && sensorValues[RIGHT+1]>800);
      i++;
      maneuver(0,0,0,0);
      delay(100);
     }
    ...
    while(digitalRead(LEFT_IR)==BLACK || digitalRead(RIGHT_IR)==BLACK)
    lineFollow();
    maneuver(FORWARD,FORWARD,225,225);
    delay(300);
    maneuver(0,0,0,0);
